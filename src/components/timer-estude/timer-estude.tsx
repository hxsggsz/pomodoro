  ...
  return (
    <StyledWrapper>
      ...
      <StyledTimer>
        ...
        <motion.p initial={{ y: -10 }} animate={{ y: 0 }} exit={{ y: -75 }} className='timer'>{minutes && minutes < 10 ? "0" + minutes : minutes} : {seconds < 10 ? "0" + seconds : seconds}</motion.p>


        <StyledWrapButtons>

          <motion.button whileHover={{ opacity: 0.6, y: -2, }} whileTap={{ y: [4, -4, 4] }} transition={{ delay: 0.1, type: "spring" }} className="button" onClick={handlePause}>{pause ? 'Começar' : 'Pausar'}</motion.button>

          <motion.button whileHover={{ opacity: 0.6, y: -2, }} whileTap={{ y: [4, -4, 4] }} transition={{ delay: 0.1, type: "spring" }} className="button" onClick={handleRestart}>Reiniciar</motion.button>
